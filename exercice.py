...
def convert_to_samples(bytes):
	...
	for i in range(0, len(bytes), 2):
		...
		integer_sample = struct.unpack("h", encoded_sample)[0]
...
def main():
	print([int(b) for b in struct.pack("h", 257)])
	print(struct.unpack("h", bytes([1, 1])))

	print(merge_channels([[11, 12], [21, 22]]))
	print(separate_channels([11, 21, 12, 22, 13, 23], 2))
